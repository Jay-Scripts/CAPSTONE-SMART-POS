...
try {
    ...
    $rows = $conn->query("
        SELECT ii.item_id, ii.item_name, ii.quantity, ii.unit, ii.status, ii.date_made, ii.date_expiry, s.staff_name, c.category_name
        FROM inventory_item ii
        JOIN staff_info s ON ii.added_by = s.staff_id
    ...
    $materials = $conn->query("
        SELECT ii.item_id, ii.item_name, ii.quantity, ii.unit, ii.status, ii.date_made, ii.date_expiry, s.staff_name
        FROM inventory_item ii
        JOIN staff_info s ON ii.added_by = s.staff_id
...
<script>
    ...
    document.getElementById('generateTemplate').addEventListener('click', () => {
        ...
        function createSheet(items, sheetName) {
            const sheetData = [
                ['Item Name', 'Unit', 'Quantity', 'Item ID'] // Column headers
            ];
            items.forEach(item => sheetData.push([
                item.item_name || '',
                item.unit || '',
                '', // Quantity left empty for user to fill
                item.item_id || '' // Include item_id for safe mapping
            ]));
    ...
    document.getElementById('submitInventory').addEventListener('click', function() {
        ...
        reader.onload = function(e) {
            ...
            workbook.SheetNames.forEach(sheetName => {
                ...
                items.forEach(item => {
                    // Match by item_id instead of item_name
                    const row = rows.find(r => r[3] == item.item_id); // 4th column = item_id
                    const actual = row ? parseFloat(row[2]) || 0 : 0; // Quantity column
                    ...
                    inventoryData.push({
                        ...
                        unit: item.unit,
                        item_id: item.item_id, // Keep for backend processing
