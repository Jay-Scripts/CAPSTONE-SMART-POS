        ...
        <form id="inventoryForm" class="space-y-4">
            ...
            <div>
                ...
                <input
                    ...
                    required
                    autocomplete="off"
                    class="w-full p-2 border rounded-lg bg-[var(--background-color)] border-[var(--border-color)]"
                    placeholder="Type or select an item name" />
                <datalist id="itemNamesList">
                    <?php foreach ($existingItemNames as $name): ?>
                        <option value="<?= htmlspecialchars($name) ?>">
                        <?php endforeach; ?>
                </datalist>
                <p class="text-xs text-gray-400 mt-1">Select existing to add a new batch, or type new to create a new item.</p>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <input type="date" id="date_made" required
                        class="w-full p-2 border rounded-lg bg-[var(--background-color)] border-[var(--border-color)]" />
                </div>
                <div>
                    <label class="block text-sm font-medium mb-1" for="date_expiry">Expiry Date</label>
                    <input type="date" id="date_expiry" required
                        class="w-full p-2 border rounded-lg bg-[var(--background-color)] border-[var(--border-color)]" />
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3 pt-4">
                ...
                <button type="submit"
                    class="bg-green-600 text-white px-4 py-2 rounded-lg">Save</button>
            </div>
