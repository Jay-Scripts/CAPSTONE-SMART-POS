...
<section class="flex justify-center p-4 sm:p-6">
                ...
                <div>
                    <?php if (!empty($category['items'][0]['item_id'])): ?>
                        <div class="overflow-x-auto m-2">
                            <table class="categoryTable w-auto text-sm text-center">
                                <thead>
                                    <tr class="border-2 border[var(--border-color)]">
                                        ...
                                        <th class="py-2 px-3 border border[var(--border-color)]">Manufacturing Date</th>
                                        <th class="py-2 px-3 border border[var(--border-color)]">Expiry Date</th>
                                ...
                                <tbody>
                                    <?php foreach ($category['items'] as $item): ?>
                                        <tr class="border-b border[var(--border-color)] hover:bg-blue-400/10 transition">
    ...
    <div id="inventoryModal" class="fixed inset-0 bg-opacity-50 hidden z-50 flex items-center justify-center p-4">
        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-sm sm:max-w-md p-6 animate-[fadeIn_0.3s_ease]">
            ...
            <form id="inventoryForm" class="space-y-4"> <!-- Item Name -->
                <div> <label class="block text-sm font-medium text-gray-700 mb-1" for="item_name">Item Name</label> <input type="text" id="item_name" required class="w-full p-2 border rounded-lg" /> </div> <!-- Inventory Category -->
                <div> <label class="block text-sm font-medium text-gray-700 mb-1" for="inv_category">Inventory Category</label> <select id="inv_category" required class="w-full p-2 border rounded-lg">
                        <option value="" disabled selected>Select inventory category</option> <?php foreach ($invCategories as $cat): ?> <option value="<?= $cat['inv_category_id'] ?>"><?= htmlspecialchars($cat['category_name']) ?></option> <?php endforeach; ?>
                    </select> </div> <!-- Product Category -->
                <div> <label class="block text-sm font-medium text-gray-700 mb-1" for="prod_category">Product Category</label> <select id="prod_category" class="w-full p-2 border rounded-lg">
                        <option value="" disabled selected>Select product category</option> <?php foreach ($prodCategories as $cat): ?> <option value="<?= $cat['category_id'] ?>"><?= htmlspecialchars($cat['category_name']) ?></option> <?php endforeach; ?>
                    </select> </div> <!-- Product -->
                <div> <label class="block text-sm font-medium text-gray-700 mb-1" for="product">Product</label> <select id="product" class="w-full p-2 border rounded-lg">
                        <option value="" disabled selected>Select product</option> <?php foreach ($products as $prod): ?> <option value="<?= $prod['product_id'] ?>"><?= htmlspecialchars($prod['product_name']) ?></option> <?php endforeach; ?>
                    </select> </div> <!-- Quantity & Unit -->
                <div class="grid grid-cols-2 gap-3">
                    <div> <label class="block text-sm font-medium text-gray-700 mb-1" for="quantity">Quantity</label> <input type="number" id="quantity" required min="1" class="w-full p-2 border rounded-lg" /> </div>
                    <div> <label class="block text-sm font-medium text-gray-700 mb-1" for="unit">Unit</label> <select id="unit" class="w-full p-2 border rounded-lg">
                            <option value="pcs">Pieces (pcs)</option>
                            <option value="g">Grams (g)</option>
                            <option value="ml">Milliliters (ml)</option>
                        </select> </div>
                </div> <!-- Date Made & Expiry -->
                <div class="grid grid-cols-2 gap-3">
                    <div> <label class="block text-sm font-medium text-gray-700 mb-1" for="date_made">Date Made</label> <input type="date" id="date_made" required class="w-full p-2 border rounded-lg" /> </div>
                    <div> <label class="block text-sm font-medium text-gray-700 mb-1" for="date_expiry">Expiry Date</label> <input type="date" id="date_expiry" required class="w-full p-2 border rounded-lg" /> </div>
                </div>
                <div class="flex justify-end gap-3 pt-4"> <button type="button" id="closeModalBtn" class="bg-gray-200 px-4 py-2 rounded-lg">Cancel</button> <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded-lg">Save</button> </div>
            </form>
        </div>
    </div>
</section>
