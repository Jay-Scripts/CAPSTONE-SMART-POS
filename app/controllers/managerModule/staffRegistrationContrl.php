...
if (isset($_POST['registerStaff'])) {
    ...
    if (empty($sanitizedStaffName)) {
        ...
        $staffRegistrationMessage['staffName'] = "<p class='text-red-500 text-sm bg-red-300 w-full rounded-xl p-3'>Staff name must be at least 3 characters longStaff name must be at least 3 characters long</p>";
    ...
    // 3️ Checking
    $hasErrors = false;
    foreach ($staffRegistrationMessage as $field => $msg) {
        if (!empty($msg)) {
            $hasErrors = true;
        }
    }
...
>
  ...
  <!-- Close button -->
  <button onclick='closeAlert()' class='ml-auto text-green-700 hover:text-green-900'>✕</button>
</div>";
